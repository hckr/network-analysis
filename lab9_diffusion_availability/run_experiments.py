...
def main():
    ...
    for experiment_name, graph, values in experiments:
        ...
        while True:
            ...
            graph = update_step(graph, values['infection'], values['removal'], values['availability'])
...
def update_step(graph: Graph, infection_probability: float, removal_probability: float, availability_probability: float):
    new_graph = graph.copy()

    for n in graph.nodes:
        if graph.nodes[n]['state'] == 'i' and random.uniform(0, 1) < availability_probability:
            for neighbor in graph.neighbors(n):
                if graph.nodes[neighbor]['state'] == 's' and \
                        random.uniform(0, 1) < infection_probability and \
                        random.uniform(0, 1) < availability_probability:
